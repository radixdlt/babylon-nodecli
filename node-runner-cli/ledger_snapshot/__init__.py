...
def download_and_extract_snapshot(data_dir):
    ...
    if not os.listdir(data_dir):
        ...
        print(
            f"Directory {data_dir} is not empty. Downloading Ledger Snapshot aborted:"
        )
...
def radix_live_download():
    ...
    if not os.path.isfile("RADIXDB-INDEX.tar.zst"):
        ...
        if response.status_code == 200:
            ...
            print(
                f"Failed to download the snapshot script. Status code: {response.status_code}"
            )
            exit(1)
            ...
            with tarfile.open(fileobj=reader, mode="r|") as tar:
                ...
                for member in tar:
                    ...
                    if time.time() - start_time >= 180:  # 3 minutes
                        print(
                            f"{total_extracted_size / (1024 * 1024):.2f} MB extracted so far."
                        )
                        start_time = time.time()
    ...
    print(
        f"Snapshot extracted successfully in {time.time() - download_start_time:.2f} seconds."
    )
