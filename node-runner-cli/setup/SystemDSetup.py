...
class SystemDSetup(BaseSetup):
    ...
    def setup_user(add_to_docker=True):
        ...
        user_exists = run_shell_command(
            "cat /etc/passwd | grep radixdlt", shell=True, fail_on_error=False
        )
    ...
    def backup_file(filepath, filename, backup_time, auto_approve=False):
        if os.path.isfile(f"{filepath}/{filename}"):
            ...
            if auto_approve is None:
                backup_yes = input(
                    f"{filename} file exists. Do you want to back up [Y/n]:"
                )
    ...
    def download_binaries(
        ...
        unzipped_folder_name = os.getenv(
            UNZIPPED_NODE_DIST_FOLDER, f"core-{node_version}"
        )
        run_shell_command(
            f"mv {unzipped_folder_name}/* {node_dir}/{node_version}", shell=True
        )
        ...
        run_shell_command(
            f"sudo mv libcorerust.so /usr/lib/jni/libcorerust.so", shell=True
        )
    ...
    @staticmethod
    def install_nginx():
        nginx_installed = run_shell_command(
            "sudo service --status-all | grep nginx", shell=True, fail_on_error=False
        )
        if nginx_installed.returncode != 0:
            run_shell_command("sudo apt install -y nginx apache2-utils", shell=True)
            run_shell_command(
                "sudo rm -rf /etc/nginx/{sites-available,sites-enabled}", shell=True
            )
    ...
    def setup_nginx_config(
        ...
        if node_type == "archivenode":
            ...
            print(
                f"Node type - {node_type} specificed should be either archivenode or fullnode"
            )
            sys.exit(1)
        ...
        if auto_approve is None:
            ...
            if Helpers.check_Yes(backup_yes):
                ...
                run_shell_command(
                    f"sudo cp -r {nginx_etc_dir} {backup_time}/nginx-config", shell=True
                )
        ...
        if Helpers.check_Yes(continue_nginx):
            ...
            run_shell_command(
                f"sudo unzip -o radixdlt-nginx.zip -d {nginx_etc_dir}", shell=True
            )
            run_shell_command(
                f"sudo mv {nginx_etc_dir}/{conf_file}  /etc/nginx/nginx.conf",
                shell=True,
            )
            run_shell_command(
                f"sudo mkdir -p /var/cache/nginx/radixdlt-hot", shell=True
            )
            return True
        else:
            return False
    ...
    def create_ssl_certs(secrets_dir, auto_approve=None):
        ...
        if os.path.isfile(f"{secrets_dir}/server.key") and os.path.isfile(
            f"{secrets_dir}/server.pem"
        ):
            if auto_approve is None:
                print(
                    f"Files  {secrets_dir}/server.key and os.path.isfile(f'{secrets_dir}/server.pem already exists"
                )
    ...
    def setup_nginx_password(secrets_dir, usertype, username, password=None):
        ...
        run_shell_command(
            f"sudo touch {secrets_dir}/htpasswd.{usertype}",
            fail_on_error=True,
            shell=True,
        )
        if password is None:
            ...
            run_shell_command(
                f"sudo htpasswd -b -c {secrets_dir}/htpasswd.{usertype} {username} {password}",
                shell=True,
            )
        print(
    ...
    def checkUser():
        ...
        result = run_shell_command(
            f"whoami | grep radixdlt", shell=True, fail_on_error=False
        )
        if result.returncode != 0:
            print(
                " You are not logged as radixdlt user. Logout and login as radixdlt user"
            )
            sys.exit(1)
        else:
    ...
    def create_initial_service_file():
        run_shell_command(
            "sudo touch /etc/systemd/system/radixdlt-node.service", shell=True
        )
        run_shell_command(
    ...
    def load_settings(config_file) -> SystemDConfig:
        if not os.path.isfile(config_file):
            print(
                f"No configuration found. Execute 'babylonnode systemd config' first."
            )
            sys.exit(1)
    ...
    @staticmethod
    def update_versions(
        systemd_config: SystemDConfig, autoapprove=False
    ) -> SystemDConfig:
        if hasattr(systemd_config, "core_node"):
            ...
            systemd_config.core_node.core_release = Prompts.confirm_version_updates(
        ...
        if hasattr(systemd_config, "gateway") and systemd_config.gateway.enabled:
            ...
            if hasattr(systemd_config.gateway, "data_aggregator"):
                systemd_config.gateway.data_aggregator.release = (
            ...
            if hasattr(systemd_config.gateway, "gateway_api"):
                systemd_config.gateway.gateway_api.release = (
                    Prompts.confirm_version_updates(
                        current_gateway_release,
                        latest_gateway_release,
                        "GATEWAY",
            ...
            if hasattr(systemd_config.gateway, "database_migration"):
                systemd_config.gateway.database_migration.release = (
                    Prompts.confirm_version_updates(
                        current_gateway_release,
                        latest_gateway_release,
                        "DATABASE MIGRATION",
        ...
        if hasattr(systemd_config.common_config, "nginx_settings"):
            ...
            systemd_config.common_config.nginx_settings.release = (
                Prompts.confirm_version_updates(
    ...
    @staticmethod
    def ask_common_config(
        argument_object: SystemDConfigArguments,
    ) -> CommonSystemdConfig:
        ...
        systemd_config.common_config.ask_enable_nginx_for_core(
    ...
    def ask_migration(argument_object: SystemDConfigArguments) -> CommonMigrationConfig:
        ...
        if "MIGRATION" in argument_object.setupmode.mode:
            systemd_config = MigrationSetup.ask_migration_config(
                systemd_config,
    ...
    def install_systemd_service(settings: SystemDConfig, args):
        ...
        SystemDSetup.confirm_config(
            settings.core_node.nodetype,
        ...
        SystemDSetup.download_binaries(
            binary_location_url=settings.core_node.core_binary_url,
        ...
        SystemDSetup.backup_file(
            settings.core_node.node_dir, f"default.config", backup_time, args.auto
        )
        settings.create_default_config_file(args.advanceduserconfig)

        # environment file
        SystemDSetup.backup_file(
            settings.core_node.node_secrets_dir, "environment", backup_time, args.auto
        )
        settings.create_environment_file()

        # radixdlt-node.service file
        SystemDSetup.backup_file(
            "/etc/systemd/system", "radixdlt-node.service", backup_time, args.auto
        )
        if args.manual:
        ...
        # Nginx
        nginx_configured = SystemDSetup.setup_nginx_service(
            settings, backup_time, args.auto
        )
    ...
    def chown_files(settings):
        ...
        run_shell_command(
            ["sudo", "chown", "radixdlt:radixdlt", key_path], fail_on_error=False
        )
    ...
    def setup_nginx_service(
        settings: SystemDConfig, backup_time: str, autoapprove: bool
    ):
        SystemDSetup.backup_file(
            "/lib/systemd/system", "nginx.service", backup_time, autoapprove
        )
        SystemDSetup.create_ssl_certs(
            settings.common_config.nginx_settings.secrets_dir, autoapprove
        )
        nginx_configured = SystemDSetup.setup_nginx_config(
            ...
            nginx_etc_dir=settings.common_config.nginx_settings.dir,
