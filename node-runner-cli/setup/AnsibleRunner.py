...
class AnsibleRunner:
    ...
    def download_ansible_file(self, file):
        ...
        if not resp.ok:
            print(
                f"{resp.status_code} error retrieving ansible playbook.. Exiting the command..."
            )
            sys.exit(1)
    ...
    def check_install_ansible(self, exit_cmd=False):
        check_ansible = run_shell_command(
            f"pip list | grep ansible", shell=True, fail_on_error=False
        )
        user = subprocess.check_output("whoami", shell=True).strip()
        if check_ansible.returncode != 0:
            print(
                f"Ansible not found for the user {user.decode('utf-8')}. Installing ansible now"
            )
    ...
    def install_ansible_modules(self):
        run_shell_command(
            f"ansible-galaxy collection install community.postgresql",
            shell=True,
            fail_on_error=True,
        )
    ...
    def run_setup_postgress(
        self, postgress_password, postgresql_user, postgresql_db_name, file
    ):
        self.check_install_ansible()
        self.download_ansible_file(file)
        run_shell_command(
            f"ansible-galaxy collection install community.postgresql", shell=True
        )
        run_shell_command(
