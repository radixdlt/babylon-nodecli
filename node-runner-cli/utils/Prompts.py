...
class Prompts:
    ...
    def ask_postgress_location(default_host):
        ...
        answer = Helpers.input_guestion(
            ...
            QuestionKeys.postgres_location,
        )
    ...
    def ask_basic_auth(target="CORE_API_NODE", user_type="admin") -> dict:
        ...
        return {
            "name": Prompts.check_default(username, user_type),
            "password": password,
        }
    ...
    def check_for_gateway() -> str:
        ...
        answer = Helpers.input_guestion(
            "Default is No[N], Press ENTER to accept default or type in [Y/N]",
    ...
    def ask_keyfile_path() -> str:
        ...
        if Helpers.check_Yes(Prompts.check_default(y_n, "N")):
            answer = Helpers.input_guestion(
                f"{bcolors.WARNING}Default is {Helpers.get_default_node_config_dir()}"
            ...
            return_value = Prompts.check_default(
                answer, Helpers.get_default_node_config_dir()
            )
            return return_value
    ...
    def ask_keyfile_name() -> str:
        ...
        value = Helpers.input_guestion(
            ...
            QuestionKeys.enter_keystore_name,
        ).strip()
        if value != "":
            keyfile_name = value
        else:
    ...
    def ask_trusted_node() -> str:
        ...
        value = Helpers.input_guestion(
            f"Fullnode requires another node to connect to network. "
    ...
    def ask_existing_compose_file(default_compose_file="docker-compose.yml"):
        ...
        y_n = Helpers.input_guestion(
            f"\nIs this first time you running the config on this machine [Y/N]:",
            QuestionKeys.first_time_config,
        )
    ...
    def get_nginx_release(latest_nginx_release) -> str:
        ...
        print(
            f"Latest release of babylon-nginx is {bcolors.OKBLUE}{latest_nginx_release}{bcolors.ENDC}."
        )
        answer = Helpers.input_guestion(
    ...
    def ask_ansible_setup_limits():
        ...
        return (
            "true"
            if Helpers.check_Yes(Prompts.check_default(ask_setup_limits, "Y"))
            else "false"
        )

    @staticmethod
    def ask_ansible_swap_setup():
        ask_setup_swap = input("Do you want to setup swap space [Y/n]?:")
        setup_swap = "false"
        ask_swap_size = None
        if Helpers.check_Yes(ask_setup_swap):
            setup_swap = "true"
            ask_swap_size = input("Enter swap size in GB. Example - 1G or 3G or 8G ?:")
    ...
    def ask_validator_address(cls) -> str:
        ...
        if Helpers.check_Yes(Prompts.check_default(answer, "N")):
            validator_address = Helpers.input_guestion(
                f"Enter your validator address:", QuestionKeys.validator_address
            )
        else:
            print(
    ...
    def ask_olympia_node_auth_password(cls) -> str:
        answer = Helpers.input_guestion(
            f"Password: ", QuestionKeys.olympia_migrations_password
        )
        return answer
    ...
    def ask_temporary_mem_limits_update(cls):
        ask = input(
            "Detected that mem_limits have not been updated for migration.\n"
    ...
    def ask_temporary_java_opts_update(cls):
        ask = input(
            "Detected that java_opts have not been updated for migration.\n"
