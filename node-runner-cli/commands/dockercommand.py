...
@dockercommand(
    [
        argument(
            "-a",
            "--autoapprove",
            help="Set this to true to run without any prompts and in mode CORE or GATEWAY."
            "Prompts still appear if you run in DETAILED mode "
            "Use this for automation purpose only",
            action="store_true",
            default=False,
        ),
        argument(
        ...
        argument(
            ...
            help="""Quick config mode with assumed defaults. It supports two quick modes and a detailed config mode.
                  ...
                  """,
            choices=["CORE", "GATEWAY", "DETAILED", "MIGRATION"],
            action="store",
        ),
        argument(
            "-miu",
        ...
        argument(
            "-miau",
            "--migration_auth_user",
            help="The user to authenticate to the olympia node for migration",
            action="store",
        ),
        argument(
            "-miap",
            "--migration_auth_password",
            help="The password to authenticate to the olympia node for migration",
            action="store",
        ),
        argument(
            "-miba",
            "--migration_bech_address",
        ...
        argument(
            "-nk",
            "--newkeystore",
            help="Set this to true to create a new store without any prompts using location"
            " defined in argument configdir",
            action="store_true",
        ),
        argument(
            "-p",
        ...
        argument(
            "-xc",
            "--disablenginxforcore",
            help="Core Node API's are protected by Basic auth setting."
        ...
        argument(
            "-xg",
            "--disablenginxforgateway",
            help="GateWay API's end points are protected by Basic auth settings. "
...
def config(args):
    ...
    if (
        "DETAILED" in argument_object.setupmode.mode
        and len(argument_object.setupmode.mode) > 1
    ):
        if (
            "CORE" in argument_object.setupmode.mode
            or "GATEWAY" in argument_object.setupmode.mode
        ):
            print(
            ...
            if (
                len(argument_object.setupmode.mode) == 2
                and "MIGRATION" in argument_object.setupmode.mode
            ):
                logger.info(f"Running modes: {argument_object.setupmode.mode}")
            else:
                logger.error(
                    f"Mode: {argument_object.setupmode.mode}. Not supported. Exiting"
                )
                sys.exit(1)
    ...
    DockerSetup.compare_config_file_with_config_object(
        argument_object.config_file, docker_config
    )
    DockerSetup.save_config(
        docker_config, argument_object.config_file, argument_object.autoapprove
    )
...
@dockercommand(
    [
        argument(
            "-a",
            "--autoapprove",
            help="Pass this option to run without any prompts. "
            "Use this for automation purpose only",
            action="store_true",
        ),
        argument(
            ...
            f"The default value is `{Helpers.get_default_node_config_dir()}/advanced-user.env-file` if not provided",
            default=f"{Helpers.get_default_node_config_dir()}/advanced-user.env-file",
            action="store",
        ),
        argument(
            "-f",
...
def install(args):
    ...
    compose_file = DockerSetup.confirm_docker_compose_file_changes(
        docker_config_updated_versions, argument_object.autoapprove
    )
...
def start(args):
    ...
    compose = DockerSetup.get_existing_compose_file(
