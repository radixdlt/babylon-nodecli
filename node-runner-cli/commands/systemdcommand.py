...
@systemdcommand(
    [
        argument(
            "-a",
            "--autoapprove",
            help="Set this to true to run without any prompts and in mode CORE."
        ...
        argument(
            "-dd",
            "--data_directory",
            help="Folder for data generated by the node",
            action="store",
        ),
        ...
        argument(
            ...
            help="""Quick config mode with assumed defaults. It supports two quick modes and a detailed config mode.
              ...
              """,
            choices=["CORE", "MIGRATION", "GATEWAY"],
            action="store",
        ),
        argument(
            "-miu",
        ...
        argument(
            "-t", "--trustednode", help="Trusted node on radix network", action="store"
        ),
        argument("-v", "--validator", help="Address of the validator ", action="store"),
        argument(
            "-x",
            "--nginxrelease",
            help="Version of radixdlt nginx release ",
            action="store",
        ),
        argument(
            "-xc",
...
def config(args):
    ...
    if (
        ...
        and systemd_config.core_node is not None
    ):
    ...
    if exists(argument_object.config_file):
        SystemDSetup.compare_old_and_new_config(
            argument_object.config_file, systemd_config
        )
    SystemDSetup.save_config(
        systemd_config, argument_object.config_file, autoapprove=args.autoapprove
    )
...
@systemdcommand(
    [
        argument(
            "-a",
            "--auto",
            help="Automatically approve all Yes/No prompts",
            action="store_true",
        ),
        argument(
            "-u",
            "--update",
            help="Update the node to new version of node",
            action="store_true",
        ),
        argument(
            "-f",
        ...
        argument(
            "-m",
            "--manual",
            help="Only generate systemd file but not put it into systemd folder."
...
def install(args):
    ...
    settings_updated_versions = (
        SystemDSetup.update_versions(settings, args.auto) if args.update else settings
    )
    SystemDSetup.install_systemd_service(settings_updated_versions, args)
...
@systemdcommand(
    [
        argument(
            ...
            choices=["all", "nginx", "radixdlt-node"],
            action="store",
        )
    ]
)
...
def dependencies(args):
    ...
    if not args.skip:
        BaseSetup.dependencies(install_docker=False)
    SystemDSetup.install_java()
    SystemDSetup.setup_user(add_to_docker=False)
