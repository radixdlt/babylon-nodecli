...
from config.EnvVars import (
    GATEWAY_DOCKER_REPO_OVERRIDE,
    AGGREGATOR_DOCKER_REPO_OVERRIDE,
    MIGRATION_DOCKER_REPO_OVERRIDE,
)
...
class DatabaseMigrationConfig(BaseConfig):
    def __init__(self, config_dict: dict):
        if config_dict is None:
            config_dict = dict()
        self.release: str = ""
        self.repo: str = os.getenv(
            MIGRATION_DOCKER_REPO_OVERRIDE, "radixdlt/babylon-ng-database-migrations"
        )
        super().__init__(config_dict)
...
class DataAggregatorConfig(BaseConfig):
    def __init__(self, config_dict: dict):
        ...
        self.release: str = ""
        self.repo: str = os.getenv(
            AGGREGATOR_DOCKER_REPO_OVERRIDE, "radixdlt/babylon-ng-data-aggregator"
        )
        self.restart: str = "unless-stopped"
        self.coreApiNode: CoreApiNodeConfig = CoreApiNodeConfig(
            config_dict.get("coreApiNode")
        )
        super().__init__(config_dict)
...
class GatewayAPIDockerConfig(BaseConfig):
    def __init__(self, config_dict: dict):
        ...
        self.release: str = ""
        self.repo: str = os.getenv(
            GATEWAY_DOCKER_REPO_OVERRIDE, "radixdlt/babylon-ng-gateway-api"
        )
        self.coreApiNode: CoreApiNodeConfig = CoreApiNodeConfig(
            config_dict.get("coreApiNode")
        )
...
class GatewayDockerConfig(BaseConfig):
    def __init__(self, config_dict: dict):
        if config_dict is None:
            config_dict = dict()
        self.data_aggregator: DataAggregatorConfig = DataAggregatorConfig(
            config_dict.get("data_aggregator")
        )
        self.gateway_api: GatewayAPIDockerConfig = GatewayAPIDockerConfig(
            config_dict.get("gateway_api")
        )
        self.postgres_db: PostgresConfig = PostgresConfig(
            config_dict.get("postgres_db")
        )
        self.database_migration: DatabaseMigrationConfig = DatabaseMigrationConfig(
            config_dict.get("database_migration")
        )
        self.enabled: bool = False
        self.docker_compose: str = (
